            ...
            <div class="modal-content">
                <form @submit.prevent="updateMatiere()">
                    <div class="modal-header">
                        <h5 class="modal-title">Modification - Matière</h5>
...
export default {
    ...
    props : ['parcours', 'niveaux', 'enseignants', 'semestres', 'matiereInfoToUpdate', 'errMsg'],
    data() {
        return {
            matiereInfo : {
                nom_matiere : "",
        ...
        matiereInfoToUpdate: {
            handler(newVal) {
                if (JSON.stringify(newVal) !== '{}') {
                    this.matiereInfo = {...newVal.matiere};
                    this.matiereInfo['parcours_id'] = (newVal.parcours).map(el => el.id);
                }
            },
            deep: true
        }
    ...
    methods: {
        async updateMatiere() {
            updateEntity('matiere', this.matiereInfo.id, this.matiereInfo)
            .then(res => { 
