...
class Matiere {
    ...
    static getByEnseignant(enseignant_id, cb) {
        connection.query(`SELECT M.*, N.nom_niveau, P.nom_parcours, E.nom_enseignant
        FROM matiere M, niveau N, parcours P, enseignant E
        WHERE E.id = ? AND M.enseignant_id = E.id AND M.parcours_id = P.id AND M.niveau_id = N.id AND M.enseignant_id = E.id`, [enseignant_id], (err, result) => {
            if (err) throw err;
            cb(result);
        })
    }

    static add(nom_matiere, niveau_id, parcours_id, enseignant_id, cb) {
    ...
    static update(id, nom_matiere, niveau_id, parcours_id, enseignant_id, cb) {
        connection.query('UPDATE matiere SET nom_matiere = ?, niveau_id = ?, parcours_id = ?, enseignant_id = ? WHERE id = ?',
            [nom_matiere, niveau_id, parcours_id, enseignant_id, id],
            (err, result) => {
                if (err) throw err;
                cb(result);
            })
