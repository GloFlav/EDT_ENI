...
export const postMatiere = (req, res) => {
    try {
        let { nom_matiere, niveau_id, parcours_id, enseignant_id } = req.body;
        Matiere.add(nom_matiere, niveau_id, (parcours_id.sort((a, b) => parseInt(a) - parseInt(b))).toString(), enseignant_id, (result) => {
            res.status(200).json({
                status: 200,
...
export const putMatiere = (req, res) => {
    try {
        let id = req.params.id;
        let { nom_matiere, niveau_id, parcours_id, enseignant_id } = req.body;
        Matiere.update(id, nom_matiere, niveau_id, (parcours_id.sort((a, b) => parseInt(a) - parseInt(b))).toString().toString(), enseignant_id, (result) => {
            res.status(200).json({
                status: 200,
                insertId: result.insertId,
